...
import com.forksystem.ui.ViewTipoPGT;
import com.forksystem.utils.Context;
import com.forksystem.utils.Utilitario;

public class TipoPagamentoController {

	public ViewTipoPGT gui;
	private ITipoPagamento dao;
	...
	public TipoPagamentoController() {
		...
		dao = Context.getInstace().getContexto().getBean(ITipoPagamento.class);
		initializeTable();
	...
	public void initializeTable() {
		...
		for (TipoPagamento dados : dao.findAll()) {
	...
	public void adicionar() {
		...
		tipo.setDescricao(gui.getTextDescricao().getText());
	...
	public void preencherCampos(TipoPagamento tipo) {
		gui.getTextId().setText(tipo.getId()+"");
		gui.getTextDescricao().setText(tipo.getDescricao().toString());
		}
	...
	public class OuvirObjecto implements ActionListener, MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			Long tipo = (Long) gui.getTable().getValueAt(gui.getTable().getSelectedRow(), 0);

			preencherCampos(dao.findOne(tipo));
			dados.add("editar");
			dados.add("apagar");
			dados.add("novo");
			disableOrenableBtnToEdit(false);
