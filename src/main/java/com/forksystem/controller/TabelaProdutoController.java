...
public class TabelaProdutoController {
	...
	private ViewOrcamento orcamento = null;
	...
	TableModelVenda model=null;
	TableModelOrcamento modelOrcamento=null;

	public TabelaProdutoController(JInternalFrame frame) {
		gui = new ViewTabelaProduto();
		
		if(frame instanceof ViewVenda){
		venda = (ViewVenda) frame;
		 model = (TableModelVenda) venda.getTable().getModel();

		}else{
			orcamento=(ViewOrcamento)frame;
			 modelOrcamento = (TableModelOrcamento) orcamento.getTable().getModel();
	...
	public void preencherCampos(Produto p) {
	   ...
	   if(null!=model){
		for (int j = 0; j < model.getRowCount() && achou == false; j++) {
			...
			for (int j = 0; j < modelOrcamento.getRowCount() && achou == false; j++) {
				Object objecto = modelOrcamento.getValueAt(j, 0);
				if (!"".equals(objecto)) {
