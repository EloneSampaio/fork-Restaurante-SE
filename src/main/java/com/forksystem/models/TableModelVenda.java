...
public class TableModelVenda extends AbstractTableModel {
	...
	public TableModelVenda(List<ItemPedido> bean, JTable table) {
	...
	public void mudar() {
		...
		this.addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				...
				TableModelVenda model = (TableModelVenda) tabela.getModel();
				switch (e.getType()) {
					...
					try {
						for (int r = firstRow; r <= lastRow; r++) {
							...
							if (!"0".equals(qtd)) {
								...
								int entradas = dao5.buscaByTipo(da4.findOne((Long) id), 1);
								int saidas = dao5.buscaByTipo(da4.findOne((Long) id), 2);
									if (saidas==0) {
									 disponivel = entradas;
								} else {
									 disponivel = entradas - saidas;

									if (disponivel < qtd.intValue()) {
										JOptionPane.showMessageDialog(null,
												"Produto com estoque baixo! Disponivel :" + disponivel+"  altera para uma quantidade menor");
